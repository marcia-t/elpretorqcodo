<?php 
	// ClassPaths for the Abogados class
		QApplicationBase::$ClassFile['abogados'] = __DATA_CLASSES__ . '/Abogados.class.php';
		QApplicationBase::$ClassFile['qqnodeabogados'] = __DATA_CLASSES__ . '/Abogados.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodeabogados'] = __DATA_CLASSES__ . '/Abogados.class.php';
		QApplicationBase::$ClassFile['abogadosmetacontrol'] = __DATA_META_CONTROLS__ . '/AbogadosMetaControl.class.php';
		QApplicationBase::$ClassFile['abogadosdatagrid'] = __DATA_META_CONTROLS__ . '/AbogadosDataGrid.class.php';

	// ClassPaths for the Agentes class
		QApplicationBase::$ClassFile['agentes'] = __DATA_CLASSES__ . '/Agentes.class.php';
		QApplicationBase::$ClassFile['qqnodeagentes'] = __DATA_CLASSES__ . '/Agentes.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodeagentes'] = __DATA_CLASSES__ . '/Agentes.class.php';
		QApplicationBase::$ClassFile['agentesmetacontrol'] = __DATA_META_CONTROLS__ . '/AgentesMetaControl.class.php';
		QApplicationBase::$ClassFile['agentesdatagrid'] = __DATA_META_CONTROLS__ . '/AgentesDataGrid.class.php';

	// ClassPaths for the Casilleros class
		QApplicationBase::$ClassFile['casilleros'] = __DATA_CLASSES__ . '/Casilleros.class.php';
		QApplicationBase::$ClassFile['qqnodecasilleros'] = __DATA_CLASSES__ . '/Casilleros.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodecasilleros'] = __DATA_CLASSES__ . '/Casilleros.class.php';
		QApplicationBase::$ClassFile['casillerosmetacontrol'] = __DATA_META_CONTROLS__ . '/CasillerosMetaControl.class.php';
		QApplicationBase::$ClassFile['casillerosdatagrid'] = __DATA_META_CONTROLS__ . '/CasillerosDataGrid.class.php';

	// ClassPaths for the Cedulas class
		QApplicationBase::$ClassFile['cedulas'] = __DATA_CLASSES__ . '/Cedulas.class.php';
		QApplicationBase::$ClassFile['qqnodecedulas'] = __DATA_CLASSES__ . '/Cedulas.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodecedulas'] = __DATA_CLASSES__ . '/Cedulas.class.php';
		QApplicationBase::$ClassFile['cedulasmetacontrol'] = __DATA_META_CONTROLS__ . '/CedulasMetaControl.class.php';
		QApplicationBase::$ClassFile['cedulasdatagrid'] = __DATA_META_CONTROLS__ . '/CedulasDataGrid.class.php';

	// ClassPaths for the Declaratorias class
		QApplicationBase::$ClassFile['declaratorias'] = __DATA_CLASSES__ . '/Declaratorias.class.php';
		QApplicationBase::$ClassFile['qqnodedeclaratorias'] = __DATA_CLASSES__ . '/Declaratorias.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodedeclaratorias'] = __DATA_CLASSES__ . '/Declaratorias.class.php';
		QApplicationBase::$ClassFile['declaratoriasmetacontrol'] = __DATA_META_CONTROLS__ . '/DeclaratoriasMetaControl.class.php';
		QApplicationBase::$ClassFile['declaratoriasdatagrid'] = __DATA_META_CONTROLS__ . '/DeclaratoriasDataGrid.class.php';

	// ClassPaths for the EstadoDeclaratoria class
		QApplicationBase::$ClassFile['estadodeclaratoria'] = __DATA_CLASSES__ . '/EstadoDeclaratoria.class.php';
		QApplicationBase::$ClassFile['qqnodeestadodeclaratoria'] = __DATA_CLASSES__ . '/EstadoDeclaratoria.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodeestadodeclaratoria'] = __DATA_CLASSES__ . '/EstadoDeclaratoria.class.php';
		QApplicationBase::$ClassFile['estadodeclaratoriametacontrol'] = __DATA_META_CONTROLS__ . '/EstadoDeclaratoriaMetaControl.class.php';
		QApplicationBase::$ClassFile['estadodeclaratoriadatagrid'] = __DATA_META_CONTROLS__ . '/EstadoDeclaratoriaDataGrid.class.php';

	// ClassPaths for the Estados class
		QApplicationBase::$ClassFile['estados'] = __DATA_CLASSES__ . '/Estados.class.php';
		QApplicationBase::$ClassFile['qqnodeestados'] = __DATA_CLASSES__ . '/Estados.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodeestados'] = __DATA_CLASSES__ . '/Estados.class.php';
		QApplicationBase::$ClassFile['estadosmetacontrol'] = __DATA_META_CONTROLS__ . '/EstadosMetaControl.class.php';
		QApplicationBase::$ClassFile['estadosdatagrid'] = __DATA_META_CONTROLS__ . '/EstadosDataGrid.class.php';

	// ClassPaths for the Modalidades class
		QApplicationBase::$ClassFile['modalidades'] = __DATA_CLASSES__ . '/Modalidades.class.php';
		QApplicationBase::$ClassFile['qqnodemodalidades'] = __DATA_CLASSES__ . '/Modalidades.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodemodalidades'] = __DATA_CLASSES__ . '/Modalidades.class.php';
		QApplicationBase::$ClassFile['modalidadesmetacontrol'] = __DATA_META_CONTROLS__ . '/ModalidadesMetaControl.class.php';
		QApplicationBase::$ClassFile['modalidadesdatagrid'] = __DATA_META_CONTROLS__ . '/ModalidadesDataGrid.class.php';

	// ClassPaths for the TipoTramites class
		QApplicationBase::$ClassFile['tipotramites'] = __DATA_CLASSES__ . '/TipoTramites.class.php';
		QApplicationBase::$ClassFile['qqnodetipotramites'] = __DATA_CLASSES__ . '/TipoTramites.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodetipotramites'] = __DATA_CLASSES__ . '/TipoTramites.class.php';
		QApplicationBase::$ClassFile['tipotramitesmetacontrol'] = __DATA_META_CONTROLS__ . '/TipoTramitesMetaControl.class.php';
		QApplicationBase::$ClassFile['tipotramitesdatagrid'] = __DATA_META_CONTROLS__ . '/TipoTramitesDataGrid.class.php';

	// ClassPaths for the TramitesAsignados class
		QApplicationBase::$ClassFile['tramitesasignados'] = __DATA_CLASSES__ . '/TramitesAsignados.class.php';
		QApplicationBase::$ClassFile['qqnodetramitesasignados'] = __DATA_CLASSES__ . '/TramitesAsignados.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodetramitesasignados'] = __DATA_CLASSES__ . '/TramitesAsignados.class.php';
		QApplicationBase::$ClassFile['tramitesasignadosmetacontrol'] = __DATA_META_CONTROLS__ . '/TramitesAsignadosMetaControl.class.php';
		QApplicationBase::$ClassFile['tramitesasignadosdatagrid'] = __DATA_META_CONTROLS__ . '/TramitesAsignadosDataGrid.class.php';

	// ClassPaths for the Zonas class
		QApplicationBase::$ClassFile['zonas'] = __DATA_CLASSES__ . '/Zonas.class.php';
		QApplicationBase::$ClassFile['qqnodezonas'] = __DATA_CLASSES__ . '/Zonas.class.php';
		QApplicationBase::$ClassFile['qqreversereferencenodezonas'] = __DATA_CLASSES__ . '/Zonas.class.php';
		QApplicationBase::$ClassFile['zonasmetacontrol'] = __DATA_META_CONTROLS__ . '/ZonasMetaControl.class.php';
		QApplicationBase::$ClassFile['zonasdatagrid'] = __DATA_META_CONTROLS__ . '/ZonasDataGrid.class.php';

?>