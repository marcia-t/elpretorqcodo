<?php
	 ...
	 * listing a collection of Posts objects.  It includes
	 ...
	 * 
	 */
	class PostsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Posts datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Posts object, or it can be any QQNode extending from QQN::Posts().
		 * 
		 ...
		 * sorting using two fields from the Posts object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Posts
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Posts::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Posts, given the clauses above
			$this->DataSource = Posts::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Posts-based QQNode.
		 * It will also verify that it is a proper Posts-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'posts') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "posts".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Posts()->Id;
				case 'Title': return QQN::Posts()->Title;
				case 'Body': return QQN::Posts()->Body;
				case 'Created': return QQN::Posts()->Created;
				case 'Modified': return QQN::Posts()->Modified;
				default: throw new QCallerException('Simple Property not found in PostsDataGrid content: ' . $mixContent);
