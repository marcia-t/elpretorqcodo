<?php
	 ...
	 * listing a collection of Declaratorias objects.  It includes
	 ...
	 * 
	 */
	class DeclaratoriasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Declaratorias datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Declaratorias object, or it can be any QQNode extending from QQN::Declaratorias().
		 * 
		 ...
		 * sorting using two fields from the Declaratorias object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Declaratorias
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdDeclaratoria=<?=urlencode($_ITEM->IdDeclaratoria)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdDeclaratoria))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDeclaratoria, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Declaratorias::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Declaratorias, given the clauses above
			$this->DataSource = Declaratorias::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Declaratorias-based QQNode.
		 * It will also verify that it is a proper Declaratorias-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'declaratorias') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "declaratorias".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdDeclaratoria': return QQN::Declaratorias()->IdDeclaratoria;
				case 'FechaInicio': return QQN::Declaratorias()->FechaInicio;
				case 'Estado': return QQN::Declaratorias()->Estado;
				case 'EstadoObject': return QQN::Declaratorias()->EstadoObject;
				case 'Honorarios': return QQN::Declaratorias()->Honorarios;
				case 'Timbrado': return QQN::Declaratorias()->Timbrado;
				case 'NroAbogado': return QQN::Declaratorias()->NroAbogado;
				case 'NroAbogadoObject': return QQN::Declaratorias()->NroAbogadoObject;
				case 'Contacto': return QQN::Declaratorias()->Contacto;
				case 'Observaciones': return QQN::Declaratorias()->Observaciones;
				case 'Autos': return QQN::Declaratorias()->Autos;
				case 'FechaFin': return QQN::Declaratorias()->FechaFin;
				case 'Observada': return QQN::Declaratorias()->Observada;
				default: throw new QCallerException('Simple Property not found in DeclaratoriasDataGrid content: ' . $mixContent);
