<?php
	 ...
	 * listing a collection of Cedulas objects.  It includes
	 ...
	 * 
	 */
	class CedulasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Cedulas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Cedulas object, or it can be any QQNode extending from QQN::Cedulas().
		 * 
		 ...
		 * sorting using two fields from the Cedulas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Cedulas
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdCedulas=<?=urlencode($_ITEM->IdCedulas)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdCedulas))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdCedulas, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Cedulas::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Cedulas, given the clauses above
			$this->DataSource = Cedulas::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Cedulas-based QQNode.
		 * It will also verify that it is a proper Cedulas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'cedulas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "cedulas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdCedulas': return QQN::Cedulas()->IdCedulas;
				case 'Localidad': return QQN::Cedulas()->Localidad;
				case 'Autos': return QQN::Cedulas()->Autos;
				case 'Direccion': return QQN::Cedulas()->Direccion;
				case 'Agente': return QQN::Cedulas()->Agente;
				case 'AgenteObject': return QQN::Cedulas()->AgenteObject;
				case 'FechaIngreso': return QQN::Cedulas()->FechaIngreso;
				case 'FechaSalida': return QQN::Cedulas()->FechaSalida;
