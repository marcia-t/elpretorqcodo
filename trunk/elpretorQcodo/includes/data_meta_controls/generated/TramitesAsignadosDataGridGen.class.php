<?php
	 ...
	 * listing a collection of TramitesAsignados objects.  It includes
	 ...
	 * 
	 */
	class TramitesAsignadosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TramitesAsignados datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TramitesAsignados object, or it can be any QQNode extending from QQN::TramitesAsignados().
		 * 
		 ...
		 * sorting using two fields from the TramitesAsignados object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TramitesAsignados
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdTramiteAsignado=<?=urlencode($_ITEM->IdTramiteAsignado)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdTramiteAsignado))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdTramiteAsignado, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TramitesAsignados::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TramitesAsignados, given the clauses above
			$this->DataSource = TramitesAsignados::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TramitesAsignados-based QQNode.
		 * It will also verify that it is a proper TramitesAsignados-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tramites_asignados') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tramites_asignados".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdTramiteAsignado': return QQN::TramitesAsignados()->IdTramiteAsignado;
				case 'IdAgente': return QQN::TramitesAsignados()->IdAgente;
				case 'IdAgenteObject': return QQN::TramitesAsignados()->IdAgenteObject;
				case 'NroAbogado': return QQN::TramitesAsignados()->NroAbogado;
				case 'NroAbogadoObject': return QQN::TramitesAsignados()->NroAbogadoObject;
				case 'IdEstado': return QQN::TramitesAsignados()->IdEstado;
				case 'IdEstadoObject': return QQN::TramitesAsignados()->IdEstadoObject;
				case 'FechaIngreso': return QQN::TramitesAsignados()->FechaIngreso;
				case 'FechaSalida': return QQN::TramitesAsignados()->FechaSalida;
				case 'TipoTramite': return QQN::TramitesAsignados()->TipoTramite;
