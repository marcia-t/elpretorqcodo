<?php
	 ...
	 * listing a collection of Zonas objects.  It includes
	 ...
	 * 
	 */
	class ZonasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Zonas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Zonas object, or it can be any QQNode extending from QQN::Zonas().
		 * 
		 ...
		 * sorting using two fields from the Zonas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Zonas
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdZona=<?=urlencode($_ITEM->IdZona)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdZona))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdZona, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Zonas::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Zonas, given the clauses above
			$this->DataSource = Zonas::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Zonas-based QQNode.
		 * It will also verify that it is a proper Zonas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'zonas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "zonas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdZona': return QQN::Zonas()->IdZona;
				case 'Nombre': return QQN::Zonas()->Nombre;
				case 'Activo': return QQN::Zonas()->Activo;
				case 'Observaciones': return QQN::Zonas()->Observaciones;
