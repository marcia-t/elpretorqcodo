<?php
	 ...
	 * listing a collection of Modalidades objects.  It includes
	 ...
	 * 
	 */
	class ModalidadesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Modalidades datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Modalidades object, or it can be any QQNode extending from QQN::Modalidades().
		 * 
		 ...
		 * sorting using two fields from the Modalidades object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Modalidades
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdModalidad=<?=urlencode($_ITEM->IdModalidad)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdModalidad))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdModalidad, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Modalidades::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Modalidades, given the clauses above
			$this->DataSource = Modalidades::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Modalidades-based QQNode.
		 * It will also verify that it is a proper Modalidades-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'modalidades') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "modalidades".');
