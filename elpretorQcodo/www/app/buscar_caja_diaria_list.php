...
class CajaDiariaListForm extends QForm {
	...
	protected function Form_Create() {
		...
		$this->calFecha = new QCalendarPopup($this);
		$this->calFecha->DateTime = QDateTime::Now(false);
		$this->calFecha->CssClass = 'textbox';
	...
	public function btnBuscar_click (){
		$fecha = $this->calFecha->DateTime;
		$this->lblCaption->Text = 'Caja diaria del día '.$fecha;
		$this->lblTimbrado->Text = $this->getTotalMovimientoPorIdYFecha(1, $fecha);
		$this->lblHonorarios->Text = $this->getTotalMovimientoPorIdYFecha(2, $fecha);
		$this->lblGastos->Text = $this->getTotalMovimientoPorIdYFecha(3, $fecha);
		$this->lblDeben->Text = $this->getTotalMovimientoPorIdYFecha(4, $fecha);
		$this->lblDoy->Text = $this->getTotalMovimientoPorIdYFecha(5, $fecha);
		$this->lblOtros->Text = $this->getTotalMovimientoPorIdYFecha(6, $fecha);
		$this->dtgCajaDiaria->DataSource =$this->dtgCajaDiaria->DataSource = Movimiento::QueryArray(QQ::Equal(QQN::Movimiento()->Fecha, $fecha));;
	}
	
	public function  getTotalMovimientoPorIdYFecha ($id, $date){
		$timbrado = Movimiento::QueryArray(QQ::AndCondition(
				QQ::Equal(QQN::Movimiento()->Fecha, $date),
				QQ::Equal(QQN::Movimiento()->TipoMovimientoObject->IdTipoMovimiento, $id)));
		$retorno = 0;
		foreach ($timbrado as $t){
			$retorno = $retorno + $t->Monto;
