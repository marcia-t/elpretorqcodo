<?php
	 ...
	 * listing a collection of Movimiento objects.  It includes
	 ...
	 * 
	 */
	class MovimientoDataGridGen extends QDataGrid {
		/**
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Movimiento
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdMovimiento=<?=urlencode($_ITEM->IdMovimiento)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdMovimiento))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdMovimiento, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Movimiento::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Movimiento, given the clauses above
			$this->DataSource = Movimiento::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Movimiento-based QQNode.
		 * It will also verify that it is a proper Movimiento-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'movimiento') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "movimiento".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdMovimiento': return QQN::Movimiento()->IdMovimiento;
				case 'Monto': return QQN::Movimiento()->Monto;
				case 'TipoMovimiento': return QQN::Movimiento()->TipoMovimiento;
				case 'TipoMovimientoObject': return QQN::Movimiento()->TipoMovimientoObject;
				case 'Fecha': return QQN::Movimiento()->Fecha;
				case 'Observaciones': return QQN::Movimiento()->Observaciones;
				default: throw new QCallerException('Simple Property not found in MovimientoDataGrid content: ' . $mixContent);
