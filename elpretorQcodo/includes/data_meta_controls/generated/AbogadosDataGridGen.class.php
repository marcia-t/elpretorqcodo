<?php
	 ...
	 * listing a collection of Abogados objects.  It includes
	 ...
	 * 
	 */
	class AbogadosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Abogados datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Abogados object, or it can be any QQNode extending from QQN::Abogados().
		 * 
		 ...
		 * sorting using two fields from the Abogados object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Abogados
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdAbogado=<?=urlencode($_ITEM->IdAbogado)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdAbogado))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdAbogado, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Abogados::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Abogados, given the clauses above
			$this->DataSource = Abogados::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Abogados-based QQNode.
		 * It will also verify that it is a proper Abogados-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'abogados') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "abogados".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdAbogado': return QQN::Abogados()->IdAbogado;
				case 'Nombre': return QQN::Abogados()->Nombre;
				case 'Apellido': return QQN::Abogados()->Apellido;
