<?php
	 ...
	 * listing a collection of Agentes objects.  It includes
	 ...
	 * 
	 */
	class AgentesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Agentes datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Agentes object, or it can be any QQNode extending from QQN::Agentes().
		 * 
		 ...
		 * sorting using two fields from the Agentes object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Agentes
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdAgente=<?=urlencode($_ITEM->IdAgente)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdAgente))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdAgente, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Agentes::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Agentes, given the clauses above
			$this->DataSource = Agentes::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Agentes-based QQNode.
		 * It will also verify that it is a proper Agentes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'agentes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "agentes".');
