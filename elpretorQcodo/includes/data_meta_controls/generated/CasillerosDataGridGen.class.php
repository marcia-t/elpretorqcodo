<?php
	 ...
	 * listing a collection of Casilleros objects.  It includes
	 ...
	 * 
	 */
	class CasillerosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Casilleros datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Casilleros object, or it can be any QQNode extending from QQN::Casilleros().
		 * 
		 ...
		 * sorting using two fields from the Casilleros object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Casilleros
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdCasillero=<?=urlencode($_ITEM->IdCasillero)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdCasillero))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdCasillero, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Casilleros::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Casilleros, given the clauses above
			$this->DataSource = Casilleros::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Casilleros-based QQNode.
		 * It will also verify that it is a proper Casilleros-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'casilleros') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "casilleros".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdCasillero': return QQN::Casilleros()->IdCasillero;
				case 'NroCasillero': return QQN::Casilleros()->NroCasillero;
				case 'Titular': return QQN::Casilleros()->Titular;
				case 'Observacion': return QQN::Casilleros()->Observacion;
				case 'Activo': return QQN::Casilleros()->Activo;
				default: throw new QCallerException('Simple Property not found in CasillerosDataGrid content: ' . $mixContent);
