<?php
	 ...
	 * listing a collection of TipoMovimiento objects.  It includes
	 ...
	 * 
	 */
	class TipoMovimientoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TipoMovimiento datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TipoMovimiento object, or it can be any QQNode extending from QQN::TipoMovimiento().
		 * 
		 ...
		 * sorting using two fields from the TipoMovimiento object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TipoMovimiento
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdTipoMovimiento=<?=urlencode($_ITEM->IdTipoMovimiento)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdTipoMovimiento))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdTipoMovimiento, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TipoMovimiento::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TipoMovimiento, given the clauses above
			$this->DataSource = TipoMovimiento::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TipoMovimiento-based QQNode.
		 * It will also verify that it is a proper TipoMovimiento-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tipo_movimiento') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tipo_movimiento".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdTipoMovimiento': return QQN::TipoMovimiento()->IdTipoMovimiento;
				case 'Nombre': return QQN::TipoMovimiento()->Nombre;
				default: throw new QCallerException('Simple Property not found in TipoMovimientoDataGrid content: ' . $mixContent);
