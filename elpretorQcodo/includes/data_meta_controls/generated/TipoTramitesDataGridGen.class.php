<?php
	 ...
	 * listing a collection of TipoTramites objects.  It includes
	 ...
	 * 
	 */
	class TipoTramitesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TipoTramites datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TipoTramites object, or it can be any QQNode extending from QQN::TipoTramites().
		 * 
		 ...
		 * sorting using two fields from the TipoTramites object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TipoTramites
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdTipoTramite=<?=urlencode($_ITEM->IdTipoTramite)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdTipoTramite))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdTipoTramite, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TipoTramites::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TipoTramites, given the clauses above
			$this->DataSource = TipoTramites::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TipoTramites-based QQNode.
		 * It will also verify that it is a proper TipoTramites-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tipo_tramites') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tipo_tramites".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdTipoTramite': return QQN::TipoTramites()->IdTipoTramite;
				case 'Nombre': return QQN::TipoTramites()->Nombre;
				case 'Timbrado': return QQN::TipoTramites()->Timbrado;
				case 'Honorario': return QQN::TipoTramites()->Honorario;
				case 'OtrosCostos': return QQN::TipoTramites()->OtrosCostos;
				case 'Observaciones': return QQN::TipoTramites()->Observaciones;
				case 'IdModalidad': return QQN::TipoTramites()->IdModalidad;
