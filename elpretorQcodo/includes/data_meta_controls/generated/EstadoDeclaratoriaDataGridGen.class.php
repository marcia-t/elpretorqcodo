<?php
	 ...
	 * listing a collection of EstadoDeclaratoria objects.  It includes
	 ...
	 * 
	 */
	class EstadoDeclaratoriaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this EstadoDeclaratoria datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the EstadoDeclaratoria object, or it can be any QQNode extending from QQN::EstadoDeclaratoria().
		 * 
		 ...
		 * sorting using two fields from the EstadoDeclaratoria object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from EstadoDeclaratoria
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdEstadoDeclaratoria=<?=urlencode($_ITEM->IdEstadoDeclaratoria)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->IdEstadoDeclaratoria))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdEstadoDeclaratoria, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = EstadoDeclaratoria::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from EstadoDeclaratoria, given the clauses above
			$this->DataSource = EstadoDeclaratoria::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a EstadoDeclaratoria-based QQNode.
		 * It will also verify that it is a proper EstadoDeclaratoria-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'estado_declaratoria') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "estado_declaratoria".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdEstadoDeclaratoria': return QQN::EstadoDeclaratoria()->IdEstadoDeclaratoria;
				case 'Nombre': return QQN::EstadoDeclaratoria()->Nombre;
				default: throw new QCallerException('Simple Property not found in EstadoDeclaratoriaDataGrid content: ' . $mixContent);
